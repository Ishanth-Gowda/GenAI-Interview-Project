...
const generateInterviewReportController = async (req, res) => {
    try {
        ...
        const interviewReport = await interviewReportModel.create({
            ...
            selfDescription,
            jobDescription,
            ...interviewReportByAi // here, instead of writing all other schema, we're getting it by AI as response anyway, hence we're destructuring it using spread operator
        })
...
async function generateResumePdfController(req, res) {
    try {
        ...
        res.status(isTransient ? 503 : 500).json({
            message: isTransient
                ? "The AI service is temporarily overloaded. Please try again in a moment."
                : "Something went wrong while generating the resume PDF."
        })
    }
}
